...
Since the data is not stationary, we will need to coerce it. First, we can seasonally difference the data.
```{r 3, echo=FALSE}
tsplot(diff(TOTAL_TRADE, lag=12), main="First Difference Monthly US Total Trade with Canada and Mexico, 1980-1994", ylab="2019 US Dollars, Millions", xlab="Year")
acf2(diff(TOTAL_TRADE, lag=12))
```
...
## Preliminary Analysis
Looking at the ACF/PACF plot for the differenced, logged series indicates an ARMA model is appropriate (tails off):
```{r 5, echo=FALSE}
...
- Through trial and error, we settled on an ARIMA(2,1,1) model.

```{r xd}
x <- sarima(log(TOTAL_TRADE), 2, 1, 1, details=FALSE)
x$ttable
```

## Our Model
```{r 6, results='hide', fig.keep='all'}
sarima(log(TOTAL_TRADE), 2, 1, 1)
```
...
sarima.for(log(TOTAL_TRADE), 84, 2, 1, 0)
