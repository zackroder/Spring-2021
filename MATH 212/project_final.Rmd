...
visreg(model_final, "GDP", gg=TRUE, main="Median Weekly Wages (USD) vs. GDP (USD, Billions)")
```

  + UNRATE:
  
```{r, echo=FALSE, fig.width=4,fig.height=2,fig.align='center'}
visreg(model_final, "UNRATE", gg=TRUE)
```

  + DJCA:
  
```{r, echo=FALSE, fig.width=4,fig.height=2,fig.align='center'}
visreg(model_final, "DJCA", gg=TRUE)
...
1. Given GDP = 21,494.731, UNRATE = 6.7, and DJCA = 9,732.69, we will use our model to predict MEDIAN_WEEKLY_WAGES.

```{r}
NEWDATA = data.frame(GDP=21494.731, UNRATE=6.7, DJCA=9732.69)
predict(model_final, NEWDATA, interval="predict") 
```

As we can see in the R output, we compute a fitted value of $957.91, with prediction interval of [\$933.22, \$982.60].
