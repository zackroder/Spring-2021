...
ggcorrplot(r, title="Economic Indicators", lab=TRUE) + labs(caption="Data from https://fred.stlouisfed.org/")
```
...
equatiomatic::extract_eq(model_final, use_coefs=TRUE)
```



2. Coefficient Interpretation.
  + When UNRATE and DJCA are held constant, every one billion dollar increase in GDP increases median weekly wages by $0.05.
  + When GDP and DJCA are held constant, every one unit increase in unemployment increases median weekly wages by \$13.47.
  + When GDP and UNRATE are held constant, every one unit increase in the DJCA index increases median weekly wages by \$0.01.
  
3. Conditional Relationship Plots
  + GDP:
  
```{r, echo=FALSE, fig.width=4,fig.height=2,fig.align='center'}
visreg(model_final, "GDP", gg=TRUE) + labs(title="Median Weekly Wages vs. GDP")
```

  + UNRATE:
  
```{r, echo=FALSE, fig.width=4,fig.height=2,fig.align='center'}
visreg(model_final, "UNRATE", gg=TRUE) + labs(title="Median Weekly Wages vs. UNRATE")
```

  + DJCA:
  
```{r, echo=FALSE, fig.width=4,fig.height=2,fig.align='center'}
visreg(model_final, "DJCA", gg=TRUE) + labs(title="Median Weekly Wages vs. DJCA")
...
As we see in the summary of the model, we have an R-squared of 0.9852. This means that our model explains 98.52% of the variance in MEDIAN_WEEKLY_WAGES.

## Predictions and Future Research Questions
1. Given GDP = 21,494.731, UNRATE = 6.2, and DJCA = 8,550.45, we will use our model to predict MEDIAN_WEEKLY_WAGES.

```{r}
NEWDATA = data.frame(GDP=21494.731, UNRATE=6.2, DJCA=8550.45)
predict(model_final, NEWDATA, interval="predict") 
```

As we can see in the R output, we compute a fitted value of $959.91, with prediction interval of [\$938.91, \$980.91].
...
As we can see in the summary for our model, we have an adjusted-R-squared of 0.9838. This does not have the same interpretation as R-squared; in other words, this does not say that our model accounts of 98.38% of variance in MEDIAN_WEEKLY_WAGES. However, adjusted-R-squared can be useful in comparing different possible models with varying numbers of explanatory variables.